...
func (s *MemorizeStrategy) willTryToPickByMemory(pm pickedMemory, currentTurn card.Turn) bool {
	if !pm.haveTwoMemory() {
		return false
	}
	if int(2*currentTurn)-int(pm.turnSum()) > s.TurnThreshold {
		return false
	}
	return true
}
...
func (s *MemorizeStrategy) willTryToPick(pickedAt, currentTurn card.Turn) bool {
	if pickedAt == 0 {
		return false
	}
	return int(currentTurn)-int(pickedAt) < s.TurnThreshold/2
}
